...
enum MediaPathMode : uint32_t {
  ...
  kDecryptAndDecode = 1,      // Can decrypt and decode the input.
  kDecrypt = 2,               // Can decrypt the input into clear.
  kCdmProxy = 3,              // The CdmProxy can handle the input.
  kTranscryptForCdmProxy = 4  // Can transcrypt (decrypt then re-encrypt) the
                              // input into what CdmProxy can handle.
};
...
class CDM_CLASS_API Host_10 {
 public:
  ...
  // Must be called by the CDM as the response of InitializeAudioPath() and
  // InitializeVideoPath() with the selected |mode|. If the |mode| is
  // kDecryptAndDecode, the CDM audio/video decoder has been successfully
  // initialized.
