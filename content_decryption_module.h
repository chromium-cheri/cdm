...
enum EncryptionScheme : uint32_t {
  kUnencrypted = 0,
  kFullSubsampleEncryptionUsingCtrMode,  // 'cenc' mode.
  kPartialPatternEncryptionUsingCbcMode  // 'cbcs' mode.
};
CHECK_TYPE(EncryptionScheme, 4, 4);

// The pattern used for partial pattern encryption.
struct Pattern {
...
// |mode| = kUnencrypted, the data is unencrypted.
...
// Fields have been reordered so that the compiler will not need to insert
// padding between fields in order to align fields based on their size.
...
struct InputBuffer_2 {
  ...
  // Encryption scheme. |pattern| is required if |encryption_scheme|
  // specifies partial pattern encryption.
  EncryptionScheme encryption_scheme;
  uint32_t padding;
  Pattern pattern;
};
CHECK_TYPE(InputBuffer_2, 56, 72);
...
// Deprecated: New CDM implementations should use AudioDecoderConfig.
...
typedef AudioDecoderConfig_2 AudioDecoderConfig;
...
// Deprecated: New CDM implementations should use VideoDecoderConfig.
...
// equals the sum of sizeof() all members in both 32-bit and 64-bit compiles.
// Padding has been added to keep the fields aligned. Padding is not used.
...
typedef VideoDecoderConfig_2 VideoDecoderConfig;
