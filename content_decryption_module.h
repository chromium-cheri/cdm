...
// Macro to check that type |name| does the following:
// 1. is a standard layout.
// 2. is trivial.
// 3. sizeof(name) matches the expected size in bytes. As some types contain
//    pointers, the size is specified for both 32 and 64 bit.
#define CHECK_TYPE(name, size_32, size_64)                           \
  static_assert(std::is_standard_layout<name>(),                     \
                #name " not standard_layout");                       \
  static_assert(std::is_trivial<name>(), #name " not trivial");      \
