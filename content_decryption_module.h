...
class CDM_CLASS_API ContentDecryptionModule_9 {
 public:
  ...
  // |storage_id_size| are set appropriately. If the version is not available,
  // null/zero will be provided as |storage_id| and |storage_id_size|,
  // respectively.
...
class CDM_CLASS_API Host_9 {
 public:
  ...
  // Requests a specific version of the storage ID. If |version| = 0, the latest
  // version will be returned (if one exists). The ID will be returned by the
  // host via ContentDecryptionModule::OnStorageId(). A storage ID is a stable,
  // device specific ID used by the CDM to securely store persistent data. The
  // CDM must not expose the ID outside the client device, even in encrypted
  // form.
